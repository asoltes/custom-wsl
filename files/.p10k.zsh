# Generated by Powerlevel10k configuration wizard on 2023-06-01 at 14:26 PST.
...
# 12h time, vertical separators, sharp heads, sharp tails, 1 line, sparse, many icons,
...
function prompt_aws_session() {

  
  if [[ -z "${AWS_EXPIRY_TIME}" ]]; then
    return
  fi
  ...
  if [[ "$current_time" -ge "$expiry_time" ]]; then
    p10k segment -b 203 -f 1 -t "🥺 expired "
  else
    expiry_seconds=$((expiry_time - current_time))
    total_minutes=$((expiry_seconds / 60))
    ...
    p10k segment -b 237 -f 255 -i '⏳' -t "${countdown}"
  fi
}
  ...
  typeset -g POWERLEVEL9K_TIMEWARRIOR_BACKGROUND=043
  ...
  typeset -g POWERLEVEL9K_TIME_BACKGROUND=043
